...
const Header = () => {
  ...
  React.useEffect(() => {
    const storedUser = localStorage.getItem('harmonystream_user');
    if (storedUser) {
      try {
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('harmonystream_auth'); // Clear auth flag
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-navigation glassmorphism-header" style={{borderRadius: '0px 0px'}}>
