...
const LoginForm = () => {
  ...
  const handleEmailLogin = async (e) => {
    ...
    // Simulate API call
      setTimeout(() => {
        // Removed mockCredentials check, allow any login for now
        // Success - set authentication flag and redirect to dashboard

        // Set authentication flag in localStorage
        localStorage.setItem('harmonystream_auth', 'true');

        // Save user data in localStorage (mocked here as form email and a placeholder username)
        const userData = {
          email: formData.email,
          username: formData.email.split('@')[0] // Use email prefix as username placeholder
        };
        localStorage.setItem('harmonystream_user', JSON.stringify(userData));
